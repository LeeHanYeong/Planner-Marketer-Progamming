...
1. 출생년도에 대한 리스트 `year_list`를 만든다. 첫 번째 요소는 1980부터, 1년씩 증가하는 5개의 요소를 삽입한다.
2. `year_list`의 3번째 출생년도를 인덱스를 사용해서 출력해본다.
3. `year_list`에서 가장 마지막 출생년도를 인덱스를 사용해서 출력해본다.